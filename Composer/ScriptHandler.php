...
class ScriptHandler
{
    ...
    private static function escShellArg($arg){
        return substr(escapeshellarg($arg), 1, -1);
    }
    ...
    public static function installAssets(Event $event)
    {
        ...
        static::executeCommand($event, $consoleDir, 'assets:install '.$symlink.self::escShellArg($webDir), $options['process-timeout']);
    }
...
EOF
    ...
    {
        $php = self::escShellArg(static::getPhp(false));
        $phpArgs = implode(' ', array_map('self::escShellArg', static::getPhpArguments()));
        $console = self::escShellArg($consoleDir.'/console');
        ...
        if (!$process->isSuccessful()) {
            throw new \RuntimeException(sprintf("An error occurred when executing the \"%s\" command:\n\n%s\n\n%s", self::escShellArg($cmd), self::removeDecoration($process->getOutput()), self::removeDecoration($process->getErrorOutput())));
        }
    }
    ...
    protected static function executeBuildBootstrap(Event $event, $bootstrapDir, $autoloadDir, $timeout = 300)
    {
        ...
        $cmd = self::escShellArg(realpath(__DIR__.'/../Resources/bin/build_bootstrap.php'));
        $bootstrapDir = self::escShellArg($bootstrapDir);
        $autoloadDir = self::escShellArg($autoloadDir);
        $useNewDirectoryStructure = '';
        if (static::useNewDirectoryStructure(static::getOptions($event))) {
            $useNewDirectoryStructure = self::escShellArg('--use-new-directory-structure');
        }
